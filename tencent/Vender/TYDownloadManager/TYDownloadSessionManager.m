...
#define DownloadCompleteModels @"downloadCompleteModels"
/**
 *  下载模型
 */
...
{
    ///删除缓存进度
    [self delteCacheData:downloadDirectory];
    
    if (!downloadDirectory) {
        downloadDirectory = self.downloadDirectory;
    }

    if ([self getDownloadState:downloadDirectory] == TYDownloadStateCompleted) {
        TYDownloadProgress * progress  = [self getCacheProgress:downloadDirectory];
        downloadDirectory = progress.filePath;
...
totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite
{
    ...
    ///duck
    downloadModel.progress.filePath = downloadModel.filePath;
    [self cacheProgress:downloadModel];
...
/**
 缓存进度对象
 */
- (void)cacheProgress:(TYDownloadModel *)downloadModel{
    
    NSData * data = [FastCoder dataWithRootObject:downloadModel.progress];
    
//    NSLog(@"progress = %@",@(downloadModel.progress.progress));
    [[NSUserDefaults standardUserDefaults]setObject:data forKey:downloadModel.downloadURL];
    [[NSUserDefaults standardUserDefaults]synchronize];

}
//获取缓存对象
- (TYDownloadProgress *)getCacheProgress:(NSString *)downloadURL{
    NSData * data = [[NSUserDefaults standardUserDefaults]objectForKey:downloadURL];
    TYDownloadProgress * progress = [FastCoder objectWithData:data];
    return progress;
}

- (void)delteCacheData:(NSString *)downloadURL{
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:downloadURL];
}

- (void)restoreDownload:(NSString *)url WithProgress:(TYDownloadProgressBlock)progressBlock state:(TYDownloadStateBlock)statBlock{

    // manager里面是否有这个model是正在下载
    TYDownloadModel * downloadModel = [[TYDownloadSessionManager manager] downLoadingModelForURLString:url];
    if (downloadModel) {
        [self startDownlaod:downloadModel WithProgress:progressBlock state:statBlock];
        return;
    }
    downloadModel = [[TYDownloadModel alloc]initWithURLString:url];

    if (!downloadModel.task && [[TYDownloadSessionManager manager] backgroundSessionTasksWithDownloadModel:downloadModel]) {
        ...
        if ([manager isDownloadCompletedWithDownloadModel:downloadModel]) {
            [manager deleteFileWithDownloadModel:downloadModel];
        }
        
        if (downloadModel.state == TYDownloadStateRunning) {
            [manager suspendWithDownloadModel:downloadModel];
            return;
        }
        [self startDownlaod:downloadModel WithProgress:progressBlock state:statBlock];
    }
...
- (void)startWithDownloadUrl:(NSString *)downloadUrl WithProgress:(TYDownloadProgressBlock)progressBlock state:(TYDownloadStateBlock)statBlock{
    TYDownloadSessionManager* manger  = [TYDownloadSessionManager manager];

    TYDownloadModel * downloadModel = [manger downLoadingModelForURLString:downloadUrl];
    if (downloadModel) {
        [manger suspendWithDownloadModel:downloadModel];
        return;
    }
    downloadModel = [[TYDownloadModel alloc]initWithURLString:downloadUrl];
    [self startWithDownloadModel:downloadModel progress:progressBlock state:statBlock];
}


- (void)startDownlaod:(TYDownloadModel*)downloadModel WithProgress:(TYDownloadProgressBlock)progressBlock state:(TYDownloadStateBlock)statBlock{
    
    [self startWithDownloadModel:downloadModel progress:^(TYDownloadProgress *progress) {
        if (progressBlock) {
            progressBlock(progress);
        }
    } state:^(TYDownloadState state, NSString *filePath, NSError *error) {
        if (statBlock) {
            statBlock(state,filePath,error);
        }
    }];
}
- (TYDownloadState )getDownloadState:(NSString *)downloadURL{
    
   
    TYDownloadProgress * progress = [self getCacheProgress:downloadURL];
    TYDownloadState state;
    
    if (progress.progress == 1) {
        
        state = TYDownloadStateCompleted;
        
    }else if(progress.progress >0&&progress.progress<1){
      
        TYDownloadModel * downloadModel = [self downLoadingModelForURLString:downloadURL];
        if (downloadModel) {
            state =  TYDownloadStateRunning;
        }else{
            state =  TYDownloadStateSuspended;
        }
        
    }else{
        state = TYDownloadStateNone;
    }
